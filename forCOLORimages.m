...
unregistered = imread('a.png');% read unregistered image
...
base = imread('b.png');% read refrence image
...
new_base=rgb2gray(base);
new_registered = imresize(reg, [size(new_base,1), size(new_base,2)]);
% [peaksnr,snr] = psnr(new_registered,new_base);
...
hold on
new_base = imcrop(new_base, vec);
err  = new_base - new_registered;
%err = new_base - new_registered;
peaksnr = 10*log10(256/std(im2double(err(:))));
snr = 10*log10(mean(im2double(new_base))/std(im2double(err(:))));
